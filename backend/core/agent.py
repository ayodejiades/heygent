...
Runs entirely on Google Gemini 2.5 Flash without complex LangChain wrappers.
"""
...
def retrieve_node(state: AgentState) -> AgentState:
    ...
    # If candidates were passed directly override, skip ChromaDB retrieval
...
def reason_and_score_node(state: AgentState) -> AgentState:
    ...
    prompt = """You are the reasoning core of a recommendation agent.
...
1. Cross-Domain Transfer: If the user profile consists of book reviews (Goodreads) or product reviews (Amazon), map their implicit taste preferences (e.g. analytical/historical mystery lover vs fast-paced tech gadget enthusiast) to the restaurant domain (e.g. quiet, intellectual, ambient Italian bistro vs high-energy, fast street suya spot).
2. Nigerian Cultural & Infrastructure Factors: If the user profile or reviews indicate a Nigerian context, explicitly favor candidates that suggest excellent air conditioning (AC), reliable constant power backup (generators), or a strong customer service culture.
3. Chain of Thought: In Step 1, reason step-by-step about why each candidate matches (or mismatches) their taste persona and current context.
...
{{
  ...
  "scored_items": [
    ...
    try:
        ...
        print(f"Warning: Error parsing JSON in reason_and_score: {e}. Raw content: {raw_content}")
        result_dict = {
...
def run_agent(user_profile: dict, user_context: str = "", candidates_override: List[dict] = None) -> dict:
    ...
    initial_state = {
        ...
        "search_query": "",
        "retrieved_candidates": candidates_override or [], # If passed directly, skips Chroma retrieval
        "scored_candidates": [],
        "reasoning_trace": "",
